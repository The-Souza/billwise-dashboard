...
export function RevenueExpenseChart({
  ...
  return (
    <Card className="lg:col-span-3">
      ...
      <CardContent>
        {isLoading ? (
          <Skeleton className="h-80 md:h-104 w-full rounded-lg" />
        ) : chartData.length === 0 ? (
          <div className="h-80 w-full flex flex-col items-center justify-center gap-2 text-muted-foreground">
            <TrendingUp className="h-8 w-8 opacity-30" />
            <p className="text-sm">Nenhuma movimentação neste período.</p>
          </div>
        ) : (
          <ChartContainer config={chartConfig} className="h-80 md:h-104 w-full">
